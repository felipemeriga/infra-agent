...
@respx.mock
def test_ask_llm_sends_prompt_and_returns_response(settings):
    ...
    result = ask_llm("What is the status?", settings=settings)
...
@respx.mock
def test_ask_llm_with_system_prompt(settings):
    ...
    result = ask_llm("Check logs", system="You are a diagnostics expert.", settings=settings)
    assert result == "OK"


@respx.mock
def test_ask_llm_raises_on_http_error(settings):
    respx.post("http://guardian:3000/api/ask").mock(
    ...
    with pytest.raises(httpx.HTTPStatusError):
        ask_llm("fail", settings=settings)
